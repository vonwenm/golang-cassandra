...
$CASSANDRA_BIN_HOME/bin/cqlsh $RPC_HOST --cql3 --file `find . -name trackdb-schema.txt` | tee /tmp/buildTrackStoreSchema.log
$CASSANDRA_BIN_HOME/bin/cqlsh $RPC_HOST --cql3 --file `find . -name report-schema.txt` | tee /tmp/buildReportSchema.log
